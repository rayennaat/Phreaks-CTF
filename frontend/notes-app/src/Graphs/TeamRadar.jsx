...
const TeamRadar = ({ teamId }) => {
  ...
  useEffect(() => {
    const fetchSolvedChallenges = async () => {
      try {
        ...
        // Filter challenges solved by the team
        ...
        const formattedData = allCategories.map(category => ({
          ...
          total: totalChallengesPerCategory[category] || 0,
        }));

        setData(formattedData);
        setMaxChallenges(maxChalls);
      } catch (err) {
  ...
  }, [teamId]); // Changed dependency from userId to teamId
  ...
  return (
    <div className="bg-[#1E1E1E] p-4 rounded-lg shadow-md w-full h-[497px]">
      <h2 className="mb-4 text-lg font-semibold text-center text-white">🏆 Team Performance Radar</h2>
